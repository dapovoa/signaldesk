    ...
    try {
      ...
      if (this.isChatGPTCodexBackend()) {
        const stream = streamChatGPTCodexResponse({
          ...
          body: {
            model,
            instructions: `You are an expert at analyzing screenshots to detect interview questions.
...
If you find ANY of these, respond with JSON where isQuestion=true and include the full question text. Be thorough - extract all visible text that appears to be a question or problem statement.`
      ...
      throw new Error(this.mapVisionError(error))
    }
  }

  private isChatGPTCodexBackend(): boolean {
    return this.config.baseURL?.includes('chatgpt.com/backend-api/codex') ?? false
  }

  private parseVisionOutput(content: string): VisionAnalysisResult {
    try {
  ...
  private mapVisionError(error: unknown): string {
    ...
    if (
      ...
      return 'OAuth image analysis failed due to an incompatible endpoint/challenge response. Please retry after reconnecting OAuth.'
    }
